...
test_executor() {
    ...
    time timeout --signal=9 ${duration} expect -n -c "set timeout ${duration}" -c "spawn bash -x ${SPARK_BASEDIR}examples/pxScene2d/src/pxscene-testrunner.sh" gdb-expect.txt 2>&1 | tee "${test_file}" || true
