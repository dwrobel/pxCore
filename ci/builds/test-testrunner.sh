...
check_result() {
    ...
    local failures=$(grep -e "^Failures: [0-9].*" "${log_file}" | tail -n 1 | awk '{print $2}');
...
test_executor() {
    ...
    dump_env
    test_file=$(mktemp -t pxscene-$(basename $0)-$$-XXX.log)

    configure_sanitizers

    time timeout --signal=9 ${duration} ${SPARK_BASEDIR}examples/pxScene2d/src/pxscene-testrunner.sh 2>&1 | tee "${test_file}" || true
